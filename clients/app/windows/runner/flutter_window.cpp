...
bool FlutterWindow::OnCreate() {
  ...
  native_interface_ =
      std::make_unique<NativeVpnImpl>(this, std::move(callbacks));
...
void FlutterWindow::OnDestroy() {
  if (flutter_controller_) {
    NativeVpnInterface::SetUp(flutter_controller_->engine()->messenger(),
                              nullptr);
  }
  if (auto *native = dynamic_cast<NativeVpnImpl *>(native_interface_.get())) {
    native->Shutdown();
  }
...
void FlutterWindow::RunOnUIThread(std::function<void()> task) {
  ...
  if (!PostMessageW(GetHandle(), WM_RUN_ON_UI,
                    reinterpret_cast<WPARAM>(heap_task), 0)) {
    delete heap_task;
  }
}

void FlutterWindow::DrainPendingUiTasks() {
  MSG message{};
  while (PeekMessageW(&message, GetHandle(), WM_RUN_ON_UI, WM_RUN_ON_UI,
                      PM_REMOVE)) {
