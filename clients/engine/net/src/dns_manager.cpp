...
static constexpr size_t VPN_INTERNAL_IPS_NUM = std::size(VPN_INTERNAL_DNS_IPS_V4) + std::size(VPN_INTERNAL_DNS_IPS_V6);
static const std::array<SocketAddress, VPN_INTERNAL_IPS_NUM> VPN_INTERNAL_DNS_IPS = []() {
    std::array<SocketAddress, VPN_INTERNAL_IPS_NUM> arr;
    auto ipv6_begin = std::transform(std::begin(VPN_INTERNAL_DNS_IPS_V4), std::end(VPN_INTERNAL_DNS_IPS_V4),
            arr.begin(), [](std::string_view ip) {
                return SocketAddress(ip);
            });
    std::transform(std::begin(VPN_INTERNAL_DNS_IPS_V6), std::end(VPN_INTERNAL_DNS_IPS_V6), ipv6_begin,
            [](std::string_view ip) {
                return SocketAddress(ip);
            });
    return arr;
...
static SystemDnsServers prepare_system_servers(SystemDnsServers servers) {
    auto should_ignore = [](const std::string &s) {
        ...
        addr.set_port(0);
        return std::any_of(
                std::begin(VPN_INTERNAL_DNS_IPS), std::end(VPN_INTERNAL_DNS_IPS), [&](const SocketAddress &i) {
                    return i == addr;
                });
    };

    std::erase_if(servers.main, [&](const SystemDnsServer &server) {
        return should_ignore(server.address);
    });
    std::erase_if(servers.fallback, should_ignore);
    std::erase_if(servers.bootstrap, should_ignore);
...
bool dns_manager_set_system_servers(DnsManager *self, SystemDnsServers servers) {
    ...
    const bool supplied_servers = !servers.main.empty() || !servers.fallback.empty() || !servers.bootstrap.empty();
    SystemDnsServers prepared = prepare_system_servers(std::move(servers));
    const bool only_internal_markers =
            supplied_servers && prepared.main.empty() && prepared.fallback.empty() && prepared.bootstrap.empty();
    if (only_internal_markers) {
        dbglog(logger, "Ignoring self-generated internal DNS markers");
        return true;
    }
    self->system_servers = std::move(prepared);
