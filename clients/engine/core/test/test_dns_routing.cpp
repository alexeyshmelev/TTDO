...
TEST(DnsProxyAccessorTest, ResolvesHostnameLocallyForDoqUpstream) {
    DeclPtr<VpnEventLoop, &vpn_event_loop_destroy> server_loop{vpn_event_loop_create()};
    DeclPtr<VpnNetworkManager, &vpn_network_manager_destroy> network_manager{vpn_network_manager_get()};
    MockDnsServer bootstrap_server;
    bool complete = false;
    bool unexpected = false;
    auto server_address = bootstrap_server.start(
            SocketAddress("127.0.0.1"), server_loop.get(), network_manager->socket,
            [&] {
                complete = true;
                vpn_event_loop_exit(server_loop.get(), Millis{0});
            },
            [&](std::optional<MockDnsServer::Request>, MockDnsServer::Request) {
                unexpected = true;
                vpn_event_loop_exit(server_loop.get(), Millis{0});
                return std::nullopt;
            });
    ASSERT_TRUE(server_address.has_value());
    bootstrap_server.expect({
            .request = MockDnsServer::Request{.tcp = false, .qtype = LDNS_RR_TYPE_A, .qname = "dns.example.invalid."},
            .response = MockDnsServer::Response{.answer = {"dns.example.invalid. 60 IN A 127.0.0.1"}},
    });

    DnsProxyAccessor system_proxy({
            // NOLINTNEXTLINE(bugprone-unchecked-optional-access)
            .upstreams = {{.address = server_address->str()}},
            .cert_verify_handler = {.func = cert_verify_handler},
    });
    ASSERT_TRUE(system_proxy.start());
    std::thread server_thread([&] {
        vpn_event_loop_run(server_loop.get());
    });

    std::optional<SocketAddress> resolved = system_proxy.resolve_hostname("dns.example.invalid", /*prefer_ipv6=*/false);
    server_thread.join();
    ASSERT_TRUE(resolved.has_value());
    EXPECT_EQ(resolved->host_str(), "127.0.0.1");
    EXPECT_TRUE(complete);
    EXPECT_FALSE(unexpected);

    DnsProxyAccessor doq_proxy({
            .upstreams = {{.address = "quic://dns.example.invalid:8853", .resolved_host = resolved}},
            .socks_listener_address = SocketAddress("127.0.0.1:1"),
            .cert_verify_handler = {.func = cert_verify_handler},
    });
    EXPECT_TRUE(doq_proxy.start());
    doq_proxy.stop();
    system_proxy.stop();
}

TEST(DnsProxyAccessorTest, PrefersAaaaThroughIpv6SystemProxy) {
    DeclPtr<VpnEventLoop, &vpn_event_loop_destroy> server_loop{vpn_event_loop_create()};
    DeclPtr<VpnNetworkManager, &vpn_network_manager_destroy> network_manager{vpn_network_manager_get()};
    MockDnsServer bootstrap_server;
    bool complete = false;
    bool unexpected = false;
    auto server_address = bootstrap_server.start(
            SocketAddress("::1"), server_loop.get(), network_manager->socket,
            [&] {
                complete = true;
                vpn_event_loop_exit(server_loop.get(), Millis{0});
            },
    ...
    bootstrap_server.expect({
            .request =
                    MockDnsServer::Request{
                            ...
                            .qname = "dns.example.invalid.",
                    },
            .response = MockDnsServer::Response{.answer = {"dns.example.invalid. 60 IN AAAA ::1"}},
    });
    ...
    std::optional<SocketAddress> resolved = system_proxy.resolve_hostname("dns.example.invalid", /*prefer_ipv6=*/true);
    server_thread.join();
    ASSERT_TRUE(resolved.has_value());
    EXPECT_EQ(resolved->host_str(), "::1");
    EXPECT_TRUE(complete);
    EXPECT_FALSE(unexpected);
    system_proxy.stop();
}

TEST(DnsHandlerTest, DoesNotResolveNumericProviderInPortOnlyDoqStamp) {
    dns::ServerStamp stamp{};
    stamp.proto = dns::StampProtoType::DOQ;
    stamp.server_addr_str = ":8853";
    stamp.provider_name = "127.0.0.1";
    stamp.props = dns::ServerInformalProperties{};

    EXPECT_EQ(dns_upstream_hostname(stamp.str()), std::nullopt);
}
...
public:
    ...
    void SetUp() override {
        ...
        vpn_network_manager_update_system_dns({.main = {{.address = "127.0.0.1:1"}}});
    ...
    void configure_unavailable_encrypted_dns() {
        DnsHandlerParameters parameters{
                .dns_proxy_listener_address = SocketAddress("0.0.0.0:1"),
                .dns_upstreams = {{.address = "tls://unavailable.example.invalid"}},
                .cert_verify_handler = vpn.parameters.cert_verify_handler,
        };
        ASSERT_FALSE(vpn.tunnel->dns_handler->update_parameters(std::move(parameters)));
    }
};
...
TEST_F(DnsRoutingBase, InternalMarkerUsesSystemDnsWithoutConfiguredUpstream) {
    dst = SocketAddress(VPN_INTERNAL_DNS_IPS_V4[0], utils::PLAIN_DNS_PORT_NUMBER);
    vpn.update_exclusions(VPN_MODE_GENERAL, "");

    ASSERT_NO_FATAL_FAILURE(open_connection());
    mock_system_dns_server->expect({
            .request = MockDnsServer::Request{.tcp = false, .qtype = 1, .qname = "example.com."},
    });
    ASSERT_NO_FATAL_FAILURE(raise_dns_request("example.com"));

    vpn_event_loop_exit(this->ev_loop.get(), DEFAULT_TIMEOUT);
    vpn_event_loop_run(this->ev_loop.get());
    EXPECT_EQ(system_complete, 1);
    EXPECT_EQ(system_unexpected, 0);
    EXPECT_TRUE(redirect_upstream->connections.empty());
    EXPECT_TRUE(bypass_upstream->connections.empty());
}

TEST_F(DnsRoutingBase, IPv4InternalMarkerFallsBackToIpv6SystemDns) {
    auto ipv6_server = std::make_unique<MockDnsServer>();
    int ipv6_complete = 0;
    int ipv6_unexpected = 0;
    auto ipv6_address = ipv6_server->start(
            SocketAddress("::1"), this->ev_loop.get(), this->network_manager->socket,
            [this, &ipv6_complete] {
                vpn_event_loop_exit(this->ev_loop.get(), DEFAULT_EVENT_LOOP_EXIT_TIMEOUT);
                ++ipv6_complete;
            },
            [this, &ipv6_unexpected](std::optional<MockDnsServer::Request>, MockDnsServer::Request) {
                vpn_event_loop_exit(this->ev_loop.get(), DEFAULT_EVENT_LOOP_EXIT_TIMEOUT);
                ++ipv6_unexpected;
                return std::nullopt;
            });
    ASSERT_TRUE(ipv6_address.has_value());
    // NOLINTNEXTLINE(bugprone-unchecked-optional-access)
    vpn_network_manager_update_system_dns({.main = {{.address = ipv6_address->str()}}});
    run_event_loop_once();
    ...
    vpn_event_loop_exit(this->ev_loop.get(), DEFAULT_TIMEOUT);
    vpn_event_loop_run(this->ev_loop.get());
    EXPECT_EQ(ipv6_complete, 1);
    EXPECT_EQ(ipv6_unexpected, 0);
    EXPECT_TRUE(redirect_upstream->connections.empty());
    EXPECT_TRUE(bypass_upstream->connections.empty());
}

TEST_F(DnsRoutingBase, EmptyDnsUpdateStopsPreviousSystemProxy) {
    ...
    vpn_event_loop_exit(this->ev_loop.get(), DEFAULT_EVENT_LOOP_EXIT_TIMEOUT);
    vpn_event_loop_run(this->ev_loop.get());
    EXPECT_EQ(system_complete, 0);
    EXPECT_EQ(system_unexpected, 0);
    EXPECT_TRUE(redirect_upstream->connections.empty());
    EXPECT_TRUE(bypass_upstream->connections.empty());
}

TEST_F(DnsRoutingBase, ConfiguredButUnavailableDnsDropsIncludedQuery) {
    ASSERT_NO_FATAL_FAILURE(configure_unavailable_encrypted_dns());
    vpn.update_exclusions(VPN_MODE_GENERAL, "");
...
TEST_F(DnsRoutingBase, ConfiguredButUnavailableDnsDropsInternalMarkerQuery) {
    ASSERT_NO_FATAL_FAILURE(configure_unavailable_encrypted_dns());
    dst = SocketAddress(VPN_INTERNAL_DNS_IPS_V4[0], utils::PLAIN_DNS_PORT_NUMBER);
    vpn.update_exclusions(VPN_MODE_GENERAL, "");
...
class InternalMarkerAltExclusionsRoute : public DnsRoutingBase {
public:
    void SetUp() override {
        vpn.listener_config.dns_alt_exclusions_route = true;
...
TEST_F(InternalMarkerAltExclusionsRoute, ConnectedExcludedDomainUsesSystemDns) {
    dst = SocketAddress(VPN_INTERNAL_DNS_IPS_V4[0], utils::PLAIN_DNS_PORT_NUMBER);
    vpn.update_exclusions(VPN_MODE_GENERAL, "example.com");
    ASSERT_NO_FATAL_FAILURE(open_connection());
    mock_system_dns_server->expect({
            .request = MockDnsServer::Request{.tcp = false, .qtype = 1, .qname = "example.com."},
    });
...
TEST_F(InternalMarkerAltExclusionsRoute, DisconnectedAllowedQueryUsesSystemDns) {
    dst = SocketAddress(VPN_INTERNAL_DNS_IPS_V4[0], utils::PLAIN_DNS_PORT_NUMBER);
...
TEST_F(DnsRoutingAllProxies, FailedEncryptedDnsRestartKeepsPreviousUserProxy) {
    vpn_network_manager_update_system_dns({});
    vpn_event_loop_exit(this->ev_loop.get(), Millis{0});
    vpn_event_loop_run(this->ev_loop.get());

    DnsHandlerParameters parameters{
            .dns_proxy_listener_address = SocketAddress("127.0.0.1:1"),
            .dns_upstreams = {{.address = "tls://unavailable.example.invalid"}},
            .cert_verify_handler = vpn.parameters.cert_verify_handler,
    };
    ASSERT_FALSE(vpn.tunnel->dns_handler->update_parameters(std::move(parameters)));

    TunnelAddress destination = SocketAddress(VPN_INTERNAL_DNS_IPS_V4[0], utils::PLAIN_DNS_PORT_NUMBER);
    SocketAddress source("127.0.0.1:50001");
    ...
    vpn.update_exclusions(VPN_MODE_GENERAL, "");
    ASSERT_NO_FATAL_FAILURE(raise_and_complete(event));
    user_server->expect({
    ...
    ASSERT_NO_FATAL_FAILURE(accept_and_send(event, "example.org.", LDNS_RR_TYPE_A));

    vpn_event_loop_exit(this->ev_loop.get(), DEFAULT_TIMEOUT);
    vpn_event_loop_run(this->ev_loop.get());
    EXPECT_EQ(user_complete, 1);
    EXPECT_EQ(user_unexpected, 0);
    EXPECT_EQ(system_complete, 0);
    EXPECT_EQ(system_unexpected, 0);
    EXPECT_EQ(system_ipv6_complete, 0);
    EXPECT_EQ(system_ipv6_unexpected, 0);
}
