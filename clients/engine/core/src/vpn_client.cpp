...
std::string_view VpnClient::dns_health_check_domain() const {
    return safe_to_string_view(this->upstream_config.endpoint->name);
}
...
void VpnClient::update_bypass_ip_availability() {
    IpVersionSet x;
    if (is_reachable(ag::SocketAddress{"192.0.2.1", 9})) {
        x.set(IPV4);
    }
    if (is_reachable(ag::SocketAddress{"2001:db8::1", 9})) {
        x.set(IPV6);
    }
