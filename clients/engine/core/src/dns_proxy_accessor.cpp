...
static dns::DnsProxySettings make_dns_proxy_settings(const DnsProxyAccessor::Parameters &parameters) {
    ...
    const auto &bootstrap_dns = parameters.bootstraps;
...
std::optional<SocketAddress> DnsProxyAccessor::resolve_hostname(std::string_view hostname, bool prefer_ipv6) {
    if (m_dns_proxy == nullptr || hostname.empty()) {
        return std::nullopt;
    }

    const std::array record_types = prefer_ipv6 ? std::array{dns_utils::RT_AAAA, dns_utils::RT_A}
                                                : std::array{dns_utils::RT_A, dns_utils::RT_AAAA};
    for (dns_utils::RecordType type : record_types) {
        dns_utils::EncodeResult encoded = dns_utils::encode_request({type, hostname});
        const auto *request = std::get_if<dns_utils::EncodedRequest>(&encoded);
        if (request == nullptr) {
            continue;
        }
        Uint8Vector response = m_dns_proxy->handle_message_sync({request->data.data(), request->data.size()}, nullptr);
        dns_utils::DecodeResult decoded = dns_utils::decode_packet({response.data(), response.size()});
        const auto *reply = std::get_if<dns_utils::DecodedReply>(&decoded);
        if (reply == nullptr) {
            continue;
        }
        for (const dns_utils::AnswerAddress &answer : reply->addresses) {
            SocketAddress address({answer.ip.data(), answer.ip.size()}, 0);
            if (address.valid()) {
                return address;
            }
        }
