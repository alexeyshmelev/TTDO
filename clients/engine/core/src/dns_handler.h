...
/*
  ...
  2. If the queried domain name is EXCLUDED:
     ...
     b. If `DnsHandlerParameters::alt_exclusions_route` is `true`, the DNS message is sent to its original
        ...
        marking the connection request so that Tunnel routes it through the bypass upstream. TrustTunnel internal
        DNS marker destinations are always sent to the system DNS proxy instead.

  3. If the queried domain name is INCLUDED, and there aren't any user-configured DNS upstreams,
     a message addressed to a TrustTunnel internal DNS marker is forwarded to the system DNS proxy. A message
     addressed to any other DNS destination is sent to that original destination. For the latter, DnsHandler acts
     as a `ClientListener`: for each connection that it accepts as `ServerUpstream`, it opens a connection with the
     same protocol, source and destination addresses, app name, etc., as `ClientListener`. Tunnel knows to route
     this connection through ServerUpstream.
  ...
  4. If the queried domain name is INCLUDED, and there are user-configured DNS upstreams, the message is forwarded
     ...
     ServerUpstream. A hostname in an encrypted DNS upstream is resolved through the captured system DNS proxy
     before this tunneled connection is created. A failed restart keeps the previous working user DNS proxy; if no
     working user DNS proxy is available, queries that require it are dropped instead of using system DNS.

  If the system DNS servers can't be determined, the system proxy remains unavailable and requests whose policy
  requires it are dropped. Initialization may continue so routes that do not require system DNS still work. No
...
std::optional<std::string> dns_upstream_hostname(std::string_view address);
