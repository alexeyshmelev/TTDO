...
#ifndef _WIN32
TEST_F(PersistentRingBufferTest, RestrictsFilePermissionsOnCreateAndReuse) {
    PersistentRingBuffer buffer(path);
    ASSERT_TRUE(buffer.append(R"({"id":1})"));
...
TEST_F(PersistentRingBufferTest, RejectsNonRegularPaths) {
    std::filesystem::path target = path + ".target";
    {
    ...
    PersistentRingBuffer linked_buffer(path);
    EXPECT_FALSE(linked_buffer.append(R"({"id":1})"));
    EXPECT_FALSE(linked_buffer.read_all().has_value());
    EXPECT_FALSE(linked_buffer.clear());
    EXPECT_EQ(std::filesystem::file_size(target), 4);
    remove_file_if_exists(path);

    ASSERT_EQ(::mkfifo(path.c_str(), S_IRUSR | S_IWUSR), 0);
    PersistentRingBuffer fifo_buffer(path);
    EXPECT_FALSE(fifo_buffer.append(R"({"id":1})"));
    EXPECT_FALSE(fifo_buffer.read_all().has_value());
    EXPECT_FALSE(fifo_buffer.clear());
    remove_file_if_exists(path);
    remove_file_if_exists(target.string());
}
#endif
