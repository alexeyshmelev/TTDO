...
impl Endpoint {
    ...
    pub fn doc_dns_upstreams() -> &'static str {
        "DNS upstreams. When empty, the operating-system DNS path is preserved; no built-in resolver is used.\n\
         One of the following kinds:\n\
           * 192.0.2.53:53 -- plain DNS\n\
           * tcp://192.0.2.53:53 -- plain DNS over TCP\n\
           * tls://dns.example.invalid -- DNS-over-TLS\n\
           * https://dns.example.invalid/dns-query -- DNS-over-HTTPS\n\
           * sdns://... -- DNS stamp (see https://dnscrypt.info/stamps-specifications)\n\
           * quic://dns.example.invalid:8853 -- DNS-over-QUIC"
    }

    pub fn doc_name() -> &'static str {
...
mod tests {
    ...
    #[test]
    fn test_dns_upstreams_doc_has_no_builtin_default() {
        let doc = Endpoint::doc_dns_upstreams();
        assert!(doc.contains("no built-in resolver"));
        assert!(!doc.contains("AdGuard DNS"));
    }
    ...
    fn test_deeplink_field_mapping() {
        ...
        assert!(!ep.anti_dpi);
        assert_eq!(ep.dns_upstreams, vec!["tls://dns.example.invalid"]);
    ...
    fn test_toml_deserialization_present_fields() {
        let toml_str = r#"
            ...
            username = "alice"
            password = "s3cr3t"
            dns_upstreams = ["tls://dns.example.invalid"]
            name = "Example VPN"
        "#;
        let ep: Endpoint = toml::from_str(toml_str).unwrap();
        assert_eq!(ep.dns_upstreams, vec!["tls://dns.example.invalid"]);
