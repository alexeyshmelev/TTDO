...
pub fn read_deeplink(path: &Path) -> Result<String, String> {
    let contents = read_protected_file(path, "deep-link")?;
    let uri = single_line(&contents, "deep-link")?;
    if !uri.starts_with("tt://") {
        return Err("Deep-link file must contain a tt:// URI".to_string());
    }
    Ok(uri.to_string())
}

pub fn read_endpoint_config(path: &Path) -> Result<String, String> {
    read_protected_file(path, "endpoint config")
}
...
mod tests {
        ...
        fn new() -> Self {
            loop {
                let path = std::env::temp_dir().join(format!(
                    "trusttunnel-client-secret-input-test-{}-{}",
                    std::process::id(),
    ...
    #[test]
    fn reads_regular_protected_files() {
        let directory = TestDirectory::new();
        let credentials = directory.write_secret("credentials", "alice:secret\n");
        let deeplink = directory.write_secret("deeplink", "tt://?AQID\n");
        let endpoint_config = directory.write_secret("endpoint.toml", "username = \"alice\"\n");

        assert_eq!(
            read_credentials(&credentials).unwrap(),
            ("alice".to_string(), "secret".to_string())
        );
        assert_eq!(read_deeplink(&deeplink).unwrap(), "tt://?AQID");
        assert_eq!(
            read_endpoint_config(&endpoint_config).unwrap(),
            "username = \"alice\"\n"
        );
    }

    #[test]
    fn rejects_invalid_deeplink_contents() {
        let directory = TestDirectory::new();
        let wrong_scheme = directory.write_secret("wrong-scheme", "https://example.invalid");
        let multiline = directory.write_secret("multiline", "tt://?first\ntt://?second");

        assert!(read_deeplink(&wrong_scheme)
            .unwrap_err()
            .contains("tt:// URI"));
        assert!(read_deeplink(&multiline)
            .unwrap_err()
    ...
    fn rejects_directory() {
        let directory = TestDirectory::new();
        assert!(read_deeplink(&directory.0)
            .unwrap_err()
    ...
    fn rejects_oversized_file() {
        let directory = TestDirectory::new();
        let path = directory.write_secret("deeplink", "tt://?AQID");
        fs::OpenOptions::new()
            .write(true)
            .open(&path)
            .unwrap()
    ...
    fn rejects_group_or_other_permissions() {
        let directory = TestDirectory::new();
        let path = directory.write_secret("endpoint.toml", "username = \"alice\"");
        fs::set_permissions(&path, fs::Permissions::from_mode(0o640)).unwrap();

        assert!(read_endpoint_config(&path)
            .unwrap_err()
            .contains("chmod 600"));
    }
}
