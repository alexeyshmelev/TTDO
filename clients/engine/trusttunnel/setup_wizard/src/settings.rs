...
use crate::user_interaction::{
    ...
    ask_for_secret_raw_line, select_variant,
};
use crate::Mode;
...
outside the VPN, or when the exclusion list contains wildcard entries (e.g. *.example.com)."#)}
...
fn build_endpoint(template: Option<&Endpoint>) -> Endpoint {
        ...
        if crate::get_mode() == Mode::Interactive && predefined_params.endpoint_config.is_none() {
            ...
            match selection {
                0 => {
                    ...
                    .map(|x| load_endpoint_config(&x))
                }
                1 => {
                    // Deep-link URI
                    let uri = ask_for_secret_raw_line("Paste deep-link URI");
    ...
    let mut x = Endpoint {
        ...
        name: endpoint_config
            .as_ref()
            .and_then(|x| empty_to_none(x.name.clone()))
            .or(opt_field!(template, name).cloned().flatten()),
        ..Default::default()
    };
...
fn load_endpoint_config(path: &str) -> EndpointConfig {
    let contents = crate::secret_input::read_endpoint_config(Path::new(path))
        .unwrap_or_else(|error| panic!("Failed to read endpoint config file:\n{error}"));
    toml::de::from_str(&contents)
        .unwrap_or_else(|error| panic!("Failed to parse endpoint config:\n{error}"))
}
...
mod tests {
    ...
    #[test]
    fn endpoint_config_maps_server_export_fields() {
...
"#,
        ...
        assert_eq!(config.client_random, "aabb/ffff");
        assert_eq!(config.name, "Operator VPN");
    }

    #[test]
    fn test_empty_dns_upstreams_summary_has_no_builtin_default() {
        let endpoint = Endpoint::default();
        let summary = EndpointSummary::new(&endpoint, &[]).to_string();
        assert!(summary.contains("DNS upstreams:       (none configured)"));
        assert!(!summary.contains("AdGuard DNS"));
    }
    ...
    fn test_deeplink_field_mapping() {
        ...
        let config = DeepLinkConfig {
            ...
            anti_dpi: true,
            dns_upstreams: vec!["tls://dns.example.invalid".to_string()],
