...
mod tests {
    ...
    fn compose_writes_endpoint_dns_upstreams() {
        let settings = test_settings(vec!["tls://dns.example.invalid".into()]);
        ...
        assert_eq!(dns.len(), 1);
        assert_eq!(dns[0].as_str().unwrap(), "tls://dns.example.invalid");
        assert_eq!(parsed["endpoint"]["name"].as_str(), Some("Operator VPN"));
    }
