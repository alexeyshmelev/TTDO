...
public:
    ...
    static Error<ConnectResultError> set_system_dns(uint32_t interface_index = 0);
    Error<ConnectResultError> prepare_endpoint_addresses();
...
private:
    ...
    std::optional<std::vector<std::vector<SocketAddressStorage>>> m_prepared_endpoint_addresses;
#ifdef _WIN32
    HMODULE m_wintun;
#endif
};
