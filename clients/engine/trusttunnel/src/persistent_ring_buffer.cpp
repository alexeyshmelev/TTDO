...
#ifndef _WIN32
static ScopedFileHandle open_regular_file(const std::string &path, int flags) {
    int fd = ::open(path.c_str(), flags | O_CLOEXEC | O_NOFOLLOW | O_NONBLOCK);
    if (fd < 0) {
        return {};
    }
    ...
    return ScopedFileHandle(fd);
}

static ScopedFileHandle open_regular_file_for_append(const std::string &path, bool *created) {
    int fd = ::open(path.c_str(), O_RDWR | O_CREAT | O_EXCL | O_CLOEXEC | O_NOFOLLOW | O_NONBLOCK, S_IRUSR | S_IWUSR);
    if (fd >= 0) {
        *created = true;
    } else if (errno == EEXIST) {
        *created = false;
        return open_regular_file(path, O_RDWR);
    } else {
        return {};
    }
...
bool PersistentRingBuffer::append(std::string_view record) {
...
#else
    bool created = false;
    ScopedFileHandle fd = open_regular_file_for_append(m_path, &created);
#endif
    if (!fd.is_valid()) {
    ...
    if (::fchmod(fd.get(), S_IRUSR | S_IWUSR) != 0) {
        int error = sys::last_error();
        errlog(g_logger, "Failed to restrict ring buffer file permissions: {} ({})", sys::strerror(error), error);
        return false;
    }
#endif
...
#else
    ScopedFileHandle fd = open_regular_file(m_path, O_RDONLY);
...
#ifndef _WIN32
    struct stat file_status{};
    if (::lstat(m_path.c_str(), &file_status) != 0) {
        if (errno == ENOENT) {
            return true;
        }
        errlog(g_logger, "Failed to inspect ring buffer file: {}", sys::strerror(sys::last_error()));
        return false;
    }
    if (!S_ISREG(file_status.st_mode)) {
        errlog(g_logger, "Refusing to remove non-regular ring buffer path");
        return false;
    }
#endif
