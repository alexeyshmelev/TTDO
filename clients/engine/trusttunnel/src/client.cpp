...
static const Logger g_logger{"TRUSTTUNNEL_CLIENT"};

TrustTunnelClient::FileHandler::FileHandler(std::string_view filename)
        : m_filename(filename)
        , m_file(open_private_log_file(m_filename)) {
}

TrustTunnelClient::FileHandler::~FileHandler() {
    if (m_file != nullptr) {
        std::fclose(m_file);
    }
}
...
Error<TrustTunnelClient::ConnectResultError> TrustTunnelClient::set_system_dns(uint32_t interface_index) {
#ifdef _WIN32
    uint32_t if_index = interface_index != 0 ? interface_index : vpn_win_detect_active_if();
    if (if_index == 0) {
    ...
    SystemDnsServers servers = std::move(result.value());
    if (servers.main.empty()) {
        warnlog(g_logger, "System DNS server list is empty");
    }
    if (!vpn_network_manager_update_system_dns(std::move(servers))) {
    ...
    if (servers.main.empty()) {
        ...
        std::string contents{std::istreambuf_iterator<char>{resolv_conf}, std::istreambuf_iterator<char>{}};
        servers = parse_resolv_conf(contents);
    }
#endif
    if (servers.main.empty()) {
        warnlog(g_logger, "System DNS server list is empty");
...
Error<TrustTunnelClient::ConnectResultError> TrustTunnelClient::prepare_endpoint_addresses() {
    std::vector<std::string> addresses;
    addresses.reserve(m_config.location.endpoints.size());
    for (const TrustTunnelConfig::Endpoint &endpoint : m_config.location.endpoints) {
        addresses.emplace_back(endpoint.address.starts_with("|") ? endpoint.address.substr(1) : endpoint.address);
    }

    static constexpr size_t RESOLVE_TIMEOUT_SECONDS = 15;
    auto resolved = resolve_endpoint_addresses(addresses, RESOLVE_TIMEOUT_SECONDS);
    if (std::ranges::all_of(resolved, [](const auto &entry) {
            return entry.empty();
        })) {
    ...
    m_prepared_endpoint_addresses = std::move(resolved);
    return {};
}
...
Error<TrustTunnelClient::ConnectResultError> TrustTunnelClient::connect_to_server() {
    ...
    static constexpr size_t RESOLVE_TIMEOUT_SECONDS = 15;
    std::vector<std::vector<SocketAddressStorage>> resolved_now;
    const std::vector<std::vector<SocketAddressStorage>> *resolved_all = nullptr;
    if (m_prepared_endpoint_addresses.has_value() && m_prepared_endpoint_addresses->size() == targets.size()) {
        resolved_all = &*m_prepared_endpoint_addresses;
    } else {
        resolved_now = resolve_endpoint_addresses(to_resolve, RESOLVE_TIMEOUT_SECONDS);
        resolved_all = &resolved_now;
    }
...
VpnListener *TrustTunnelClient::make_tun_listener(ListenerSettings listener_settings) {
    ...
    VpnOsTunnelSettings tunnel_settings = {.ipv4_address = defaults->ipv4_address,
            ...
            .dns_servers =
                    (config.change_system_dns && !effective_dns.empty()) ? defaults->dns_servers : VpnAddressArray{},
