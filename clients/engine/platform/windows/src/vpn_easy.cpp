...
static void notify_start_failed(on_state_changed_t callback, void *arg) {
    if (callback) {
        callback(arg, ag::VPN_SS_DISCONNECTED);
    }
}
...
vpn_easy_t *vpn_easy_start_ex(const char *toml_config, on_state_changed_t state_changed_cb, void *state_changed_cb_arg,
    ...
    if (auto dns_error = ag::TrustTunnelClient::set_system_dns()) {
        errlog(g_logger, "Failed to collect system DNS servers: {}", dns_error->pretty_str());
        return nullptr;
    }
    ...
    void start_async(const std::string &config, on_state_changed_t callback, void *arg) {
        if (!m_loop) {
            ...
            if (!loop.start()) {
                ...
                notify_start_failed(callback, arg);
                return;
            }
...
void vpn_easy_stop_and_wait() {
    VpnEasyManager::instance().stop_and_wait();
}
