...
- (instancetype)initWithConfig:(NSString *)config
    ...
    if (self) {
        ...
        ag::VpnCallbacks callbacks = {
                .protect_handler = [](ag::SocketProtectEvent *event) { event->result = protectSocket(event) ? 0 : -1; },
                .verify_handler =
                        [](ag::VpnVerifyCertificateEvent *event) {
                            event->result = verify_certificate(event) ? 0 : -1;
                        },
                .client_output_handler =
                        [weakSelf](ag::VpnClientOutputEvent *event) {
                            @autoreleasepool {
                                ...
                                for (size_t i = 0; i < event->packet.chunks_num; ++i) {
                                    [packet appendBytes:event->packet.chunks[i].iov_base
                                                 length:event->packet.chunks[i].iov_len];
                                }
                                [strongSelf->_tunnelFlow writePackets:@[ packet ] withProtocols:@[ @(event->family) ]];
                            }
                        },
                .state_changed_handler =
                        [stateChangeHandler](ag::VpnStateChangedEvent *event) {
                            stateChangeHandler((int)event->state);
                        },
                .connection_info_handler =
                        [connectionInfoHandler](ag::VpnConnectionInfoEvent *info) {
        ...
        self->_native_client =
                std::make_unique<ag::TrustTunnelClient>(std::move(*trusttunnel_config), std::move(callbacks));
        self->_network_monitor =
                std::make_unique<ag::AutoNetworkMonitor>(self->_native_client.get(), std::move(bound_if));
...
- (bool)prepareEndpointAddresses {
    auto error = _native_client->prepare_endpoint_addresses();
    if (error) {
        errlog(g_logger, "Failed to resolve endpoint addresses: {}", error->pretty_str());
        return false;
    }
    return true;
}
...
+ (bool)prepareSystemDns {
    auto error = ag::TrustTunnelClient::set_system_dns();
    if (error) {
        errlog(g_logger, "Failed to collect system DNS servers: {}", error->pretty_str());
        return false;
    }
    return true;
}
