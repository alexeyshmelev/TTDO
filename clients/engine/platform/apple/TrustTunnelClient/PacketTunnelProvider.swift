...
open class AGPacketTunnelProvider: NEPacketTunnelProvider {
    ...
    private let INTERNAL_DNS_SERVERS = ["198.18.53.53", "fd00::53"]
    ...
    open override func startTunnel(options: [String : NSObject]? = nil, completionHandler: @escaping ((any Error)?) -> Void) {
        ...
        if (config == nil) {
            self.completeStart(TunnelError.parse_config_failed, completionHandler: completionHandler)
            return
        }
        ...
        if !VpnClient.prepareSystemDns() {
            self.completeStart(TunnelError.start_failed, completionHandler: completionHandler)
            return
        }
            ...
            stateChangeHandler: { [weak self] state in
                ...
                case .disconnected:
                    self.clientQueue.async {
                        self.stopVpnClient()
                        if (!self.completeStart(TunnelError.start_failed, completionHandler: completionHandler)) {
                            self.cancelTunnelWithError(nil)
                        }
                    }
                case .connected:
                    self.clientQueue.async {
                        self.completeStart(nil, completionHandler: completionHandler)
                        self.reasserting = false
                    }
                case .waiting_for_network, .waiting_for_recovery, .recovering:
                    self.clientQueue.async {
        ...
        if self.vpnClient == nil || !self.vpnClient!.prepareEndpointAddresses() {
            self.vpnClient = nil
            self.completeStart(TunnelError.create_failed, completionHandler: completionHandler)
            return
        }
        ...
        let effectiveDnsUpstreams = vpnConfig.endpoint.dns_upstreams ?? vpnConfig.dns_upstreams ?? []
        if !effectiveDnsUpstreams.isEmpty {
            let dnsSettings = NEDNSSettings(servers: INTERNAL_DNS_SERVERS)
        ...
        setTunnelNetworkSettings(networkSettings) { error in
            ...
            self.clientQueue.async {
                guard let vpnClient = self.vpnClient else {
                    self.completeStart(TunnelError.create_failed, completionHandler: completionHandler)
                    return
                }
                if (!vpnClient.start(self.packetFlow)) {
                    self.stopVpnClient()
                    self.completeStart(TunnelError.start_failed, completionHandler: completionHandler)
                    return
                }
    ...
    private func processConnectionInfo(json: String) {
        self.clientQueue.async {
            self.logger.debug("Connection metadata is being stored locally")
            var fileURL: URL? {
