...
public final class FileLogger {
    ...
    private let queue = DispatchQueue(label: "org.trusttunnel.client.FileLogger", qos: .utility)
    private var fileHandle: FileHandle?
    private var currentSize: Int = 0

    private static let fallbackLogger = os.Logger(subsystem: "org.trusttunnel.client.engine",
                                                  category: "FileLogger")
    ...
    private func coordinateMove(from src: URL, to dst: URL, coordinator: NSFileCoordinator) {
        ...
        { (actualSrc, actualDst) in
            let fileManager = FileManager.default
            do {
                if fileManager.fileExists(atPath: actualDst.path) {
                    try fileManager.replaceItemAt(actualDst,
                                                  withItemAt: actualSrc,
                                                  backupItemName: nil,
                                                  options: [])
                } else {
                    try fileManager.moveItem(at: actualSrc, to: actualDst)
                }
            } catch {
