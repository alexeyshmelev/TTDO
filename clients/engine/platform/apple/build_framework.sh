...
# --match client-v* -> 0.0.0-git. Exported so `pod install` (the podspec
# resolver) and the CMake-driven VpnClientFramework build pick it up; the
# numeric core is passed to xcodebuild as build-setting overrides (the Mach-O /
# Info.plist version fields must be numeric), so no source file is patched.
if [[ -n "${TT_CLIENT_VERSION:-}" ]]; then
  VER_FULL="${TT_CLIENT_VERSION}"
else
  VER_FULL="$(git describe --tags --match 'client-v*' 2>/dev/null | sed -e 's/^client-v//')"
...
VERSION_ARGS=(MARKETING_VERSION="${VER_CORE}" CURRENT_PROJECT_VERSION="${VER_CORE}" DYLIB_CURRENT_VERSION="${VER_CORE}")
